...
export function InventoryBubble({ ingredient, onOpenEditor, onOpenContextMenu }: InventoryBubbleProps) {
  ...
  return (
    <button
      ...
      {...attributes}
      className={clsx(
        'relative flex w-full items-center justify-between gap-3 rounded-full border px-3 py-2 text-left text-sm font-semibold shadow-bubble transition',
    ...
    >
      ...
      <span className="flex items-center gap-1">
        {ingredient.pinned && <span className="rounded bg-slate-900 px-1 text-[10px] text-white">PIN</span>}
        <span className="rounded-full bg-white/80 px-2 py-0.5 text-xs">x{ingredient.count}</span>
      </span>
    </button>
  );
}
